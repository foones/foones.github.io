...
def gen(index, text):
    white = (255, 255, 255)
    black = (0, 0, 0)
    dummy = PIL.Image.new('RGB', (1, 1), white)
    fnt = PIL.ImageFont.truetype('/usr/share/fonts/TTF/CourierNew.ttf', 40)
    dummyD = PIL.ImageDraw.Draw(dummy)
    width, height = dummyD.textsize(text, fnt)
    img = PIL.Image.new('RGB', (width, height), white)
    imgD = PIL.ImageDraw.Draw(img)
    imgD.text((0, 0), text, font=fnt, fill=black)
    ...
    img.save('sal.png')
    #os.system('convert sal.png -wave 3x100 -gravity North sal.png')
    os.system('convert sal.png -fill none -stroke \'' + colors[index] + '\' -strokewidth 2 -draw "polyline ' + ','.join(map(str, points)) + '" sal.png')
    k1 = random.choice([2.5, 4, 10])
    displacement = int(height / k1)
    k2 = random.choice([1, 1.2, 1.4])
    period = int(width / k2)
    rot = random.choice([-3, -2, -1, 0, 1, 2, 3])
    swirl = random.choice([-10, -5, -2, 2, 5, 10])
    os.system('convert sal.png -wave 3x100 -gravity North sal.png')
    os.system('convert sal.png -wave %ux%u -gravity North sal.png' % (displacement, period,))
    os.system('convert sal.png -swirl 10 sal.png')
    os.system('convert sal.png -implode .1 sal.png')
    os.system('convert sal.png -rotate %u sal.png' % (rot,))
    #os.system('convert sal.png +noise poisson sal.png')
    os.system('convert sal.png +noise Gaussian sal.png')
    os.system('convert sal.png -bordercolor \'' + colors[index] + '\' -border 5 sal.png')
...
for i in range(len(verses)):
    ...
    for j in range(len(poem)):
        verse = poem[j]
        if verse == "": continue
        fn = 'captcha/f%u_%u.png' % (i, j)
        print(i, j, poem[j])
        gen(i, poem[j])
        os.system('mv sal.png ' + fn)
        #sys.exit(0)
